...
require_once __DIR__ . "/forms/create_group_form.php";
$form = new \local_learningcompanions\create_group_form(null, [
        'cmid' => $cmid, 'courseid' => $courseid
    ]);
if ($data = $form->get_data()) {
    try {
        $groupid = \local_learningcompanions\groups::group_create($data->name, $data->description, $data->closedgroup, $data->keywords, $data->courseid, $data->cmid, $data->groupimage);
        ...
        $warning = new \core\output\notification(get_string('error_group_creation_failed', 'local_learningcompanions', $e->getMessage()), \core\output\notification::NOTIFY_ERROR);
    }
